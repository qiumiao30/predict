...
class Predictor:
    ...
    def get_score(self, values):
        ...
        anomaly_scores = np.max(anomaly_scores, 1) # 均值
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        e_eval = epsilon_eval(train_anomaly_scores, test_anomaly_scores, true_anomalies, self.k, reg_level=self.reg_level)
        p_eval = pot_eval(train_anomaly_scores, test_anomaly_scores, true_anomalies, self.k,
                          q=self.q, level=self.level, dynamic=self.dynamic_pot)
        if true_anomalies is not None:
            bf_eval = bf_search(test_anomaly_scores, true_anomalies, self.k, start=0.01, end=2, step_num=100, verbose=False)
        else:
            bf_eval = {}
