...
if __name__ == "__main__":
    ...
    if dataset == 'SMD':
        ...
        output_path = f'output/{dataset}'
        (x_train, _), (x_test, y_test) = get_data(f"omi-{index}", normalize=normalize)
    else:
    ...
    if args.model == "CNN_GRU":
        ...
        model = ForcastConvTransformer(
            k=n_features, headers=header, depth=depth, seq_length=window_size, kernel_size=kernel_size, mask_next=True, mask_diag=False,
            dropout_proba=dropout, num_tokens=None
        )
    else:
        pass
    # print(model)
    ...
    reg_level_dict = {"SMAP": 0, "MSL": 0, "SMD-1": 1, "SMD-2": 1, "SMD-3": 1, "WADI": 1, "SWAT":1, "WIND":1, "OMI":1}
