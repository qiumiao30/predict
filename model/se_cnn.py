...
#     expansion = 1
#
#     def __init__(self, inplanes, planes, out_dim, window, gru_hid_dim, gru_n_layers, fc_hid_dim, fc_n_layers, recon_hid_dim,
#                  recon_n_layers, downsample=None, stride=1, scales=6, groups=1, se=False, norm_layer=None):
...
#         self.stride = stride
#         self.scales = scales
#         self.gru = nn.GRU(inplanes, gru_hid_dim, batch_first=True)
...
class Res2NetBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.randn(x.shape[0], self.window, out_odd.shape[2])
        n = self.window
        out[:, 0:n:2, :] = out_odd
        out[:, 1:n:2, :] = out_even
        out = out.permute(0,2,1)
